...
text1 = grep(".{1}o+",as.list(phrases), value=T)
...
punct = grep("[[:punct:]]", phrases)
no.punct = length(punct)
...
#even <- <your code here>
even = paste("even", seq(2,1000,by=2),sep="")
...
hotelCal.split = unlist(strsplit(hotelCal.split, split= "[[:punct:]]"))
hotelCal.split = unlist(strsplit(hotelCal.split, split=" "))
hotelCal.split = hotelCal.split[hotelCal.split != ""]
