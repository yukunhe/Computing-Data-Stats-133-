...
# < your code here>
mtcars=data.frame(mtcars,rownames(mtcars))
colnames(mtcars)[12]="model"
...
optionlist <- list(hAxis="{title:'Displacement'}", vAxis="{title:'MPG'}",title= "Car Model vs. Car Displacement")

# bub <- gvisBubbleChart( < your code here >)
bub <- gvisBubbleChart(data=mtcars, idvar="model",xvar="disp",yvar="mpg",sizevar="hp",colorvar="gear",options=optionlist)
...
# < your code here >
WorldDat = WorldDat[sapply(WorldDat, function(x) any(is.na(x))==F),]
...
Motion <- gvisMotionChart(data=WorldDat,idvar="country",xvar="fertility.rate",yvar="life.expectancy",timevar="year",colorvar="region",sizevar="population")
