...
year_1998 <-new_year[wr1500m$times == min(wr1500m$times)]

# Your ggplot command:

ggplot(wr1500m,aes(wr1500m$new_year, wr1500m$times_sec))+xlim(1890,2020)+
  geom_step()+geom_segment(aes(x=year_1998, y=wr_1998,xend=2015,yend=wr_1998))
...
wr_1944<- times_sec[wr1500m$year == 1944]
year_1944 <- new_year[wr1500m$year == 1944]

# Your ggplot command

ggplot(wr1500m,aes(wr1500m$new_year, wr1500m$times_sec))+xlim(1890,2020)+
  geom_step()+geom_segment(aes(x=year_1998, y=wr_1998,xend=2015,yend=wr_1998))+
  geom_vline(aes(xintercept=year_1944), col="green")+
  geom_vline(aes(xintercept=year_1998), col="green")+
  annotate("text",year_1944,wr_1944,col="blue",label= wr1500m$athlete[wr1500m$new_year == year_1944])+
  annotate("text",year_1998,wr_1998,col="blue",label= wr1500m$athlete[wr1500m$new_year == year_1998])
...
ggplot(wr1500m,aes(wr1500m$new_year, wr1500m$times_sec))+xlim(1890,2020)+
  ...
  annotate("text",year_1944,wr_1944,col="blue",label= wr1500m$athlete[wr1500m$new_year == year_1944])+
  annotate("text",year_1998,wr_1998,col="blue",label= wr1500m$athlete[wr1500m$new_year == year_1998])+
  labs(x="Year", y="Times",title="World Record in Men's 1500 meter run")
...
ggplot(SO2012Ctry,aes(SO2012Ctry$pop, SO2012Ctry$GDP))+geom_point()
...
ggplot(SO2012Ctry,aes(SO2012Ctry$pop, SO2012Ctry$GDP_per_person))+
  geom_point(aes(size=sqrt(SO2012Ctry$Total)/40))+coord_trans(x="log10",y="log10")
...
ggplot(SO2012Ctry,aes(SO2012Ctry$pop, SO2012Ctry$GDP_per_person))+
  geom_point(aes(size=sqrt(SO2012Ctry$Total)/40))+coord_trans(x="log10",y="log10")+
  labs(x="Log of Population", y="Log of GDP per Person",title="Medal count by GDP and Population")+
  annotate("text",SO2012Ctry$pop[SO2012Ctry$Country=="China"], SO2012Ctry$GDP_per_person[SO2012Ctry$Country=="China"],col="red",label="China")+
  annotate("text",SO2012Ctry$pop[SO2012Ctry$Country=="Russia"], SO2012Ctry$GDP_per_person[SO2012Ctry$Country=="Russia"],col="blue",label="Russia")+
  annotate("text",SO2012Ctry$pop[SO2012Ctry$Country=="United States"], SO2012Ctry$GDP_per_person[SO2012Ctry$Country=="United States"],col="Yellow",label="United States")+
  annotate("text",SO2012Ctry$pop[SO2012Ctry$Country=="Germany"], SO2012Ctry$GDP_per_person[SO2012Ctry$Country=="Germany"],col="orange",label="Germany")+
  annotate("text",SO2012Ctry$pop[SO2012Ctry$Country=="UnitedKingdom"], SO2012Ctry$GDP_per_person[SO2012Ctry$Country=="UnitedKingdom"],col="Green",label="UnitedKingdom")
...
ggplot()+
  geom_polygon(aes(long,lat,group=group),fill="grey40",data=map_data("world"))
...
ggplot()+
  geom_polygon(aes(long,lat,group=group),fill="grey40",data=map_data("world"))+
  geom_point(data=wonMedal,aes(longitude,latitude,size=sqrt(Total)/40),
             colour="gold")
...
ggplot(athletes, aes(Sport, fill=Sex))+geom_bar(position="dodge",stat="bin")
...
ggplot(athletes, aes(Sport, fill=Sex))+geom_bar(position="dodge",stat="bin")+
  ...
  labs(title="Olympic Sport Participation by Gender",y="Sex")
