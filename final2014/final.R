...
set.seed(344)
m <- matrix(rexp(100,3), byrow=F,ncol=10)
...
# Color the plotting symbol by Species (any 3 colors)
plot(1:length(iris$Petal.Length),iris$Sepal.Length/iris$Petal.Length, col=as.numeric(iris$Species))
...
plot(x=infants$bwt, y=infants$gestation, pch="*", col="red",xlab="birthweight",ylab="gestation")
abline(v=259)
...
max.petal.width <- c(iris$Petal.Width[iris$Petal.Length == max(iris$Petal.Length)&iris$Species == "setosa"]), iris$Petal.Width[iris$Species == "virginica"]),max(iris$Petal.Length[iris$Species == "versicolor"]))
#################################################################
...
longerRange <- function(m1, m2){
    vector.m1 = as.vector(m1)
    vector.m2 = as.vector(m2)
    range.m1 = max(vector.m1)-min(vector.m1)
    range.m2 = max(vector.m2)- min(vector.m2)
    if (range.m1 >= range.m2) {
      return (1)
    } else {
      return(2)
    }
...
dice_sum <- function(k, B){
  replicate(sum(sample(1:6, k, replace=T),B)
...
for (i in 1:4) {
  ave.diceRoll[i]=mean(dice_sum(2,Bvec[i]))
  sd.diceRoll[i]= sd(dice_sum(2,Bvec[i]))
  hist(dice_sum(2,Bvec[i]),xlab="",main=paste("", Bvec[i]))
}
...
minchin.split <- tolower(unlist(strsplit(minchin, split=" ")))
