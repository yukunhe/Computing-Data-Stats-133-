...
#Outline
In this lab we will we will learn how to fit linear models in R. We will also cover some model selection techniques and see how to check a model is a reasonable approximation of reality.
...
They seeem to be a reasonable match; however in practice fitting complicated models can often be avoided by making a suitable transformation. For example if treat 1/mpg as the outcome, things look very linear. 
```{r, echo = FALSE}
ggplot(mtcars) + geom_point(aes(x = disp, y = 1/mpg)) + theme_bw()
```


It is important to check that your model describes reality. ``R`` provides built in plots to check some aspects of the suitability of your model
```{r}
par(mfrow = c(2,2))
plot(mpg_model)
```
