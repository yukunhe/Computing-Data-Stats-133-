...
#Sometimes, we may end up with an infinite loop, if the condition clause
#in the while statement is always true.

#Example 5.4

printOneToInf <- function() {
  i <- 1 
  while(i > 0) {
    print(i)
    i <- i + 1
...
#Please hit the stop sign in the console to end the loop.
...
boot_mean <- rep(0,100)
for (i in 1:n){
  new_ind <- sample(x=nrow(iris), size=nrow(iris), replace=T)
  dat <- iris$Sepal.Length[new_ind]
