...
n <- strsplit(tempDates, split=" ")
speechYr <- as.numeric(sapply(n, "[",3))
speechMo <- sapply(n, "[",1)
...
for(i in 1:n.speeches){
  m = paste(speeches[(breaks[i]+6):(breaks[i+1]-1)],sep=" ",collapse=" ")
  speechesL[[i]]= unlist(strsplit(m, "[.?!]"))
}
...
# Output : words, a character vector where each element is one word 
  ...
  str = gsub("\\[applause.\\]","",str)
  str = gsub("\\[laughter.\\]","",str)
  
  # Split the text up by blanks and punctuation  (hint: strsplit, unlist)
  words = unlist(strsplit(str,"[[:punct:][:blank:]]+"))
...
speechesDF <- data.frame(speechesDF, yr = speechYr, month = speechMo, words,chars,sentences)
...
docFreq <- sapply(1:length(uniqueWords), function(x)(sum(presidentWordMat[x,] != 0)))
...
presParty <- speechesDF$party[sort(order(speechesDF$Pres)[!duplicated(sort(speechesDF$Pres))])]
...
text(mds, labels = unique(speechesDF$initial),col = cols[presParty[names(presParty)]],cex=0.8)
...
plot(speechesDF$yr, speechesDF$word, xlab="Year", ylab="#words")
plot(speechesDF$yr, speechesDF$char, xlab="Year", ylab="#characters")
plot(speechesDF$yr, speechesDF$char/speechesDF$word, xlab="Year", ylab="ave word length")
plot(speechesDF$yr, speechesDF$word/speechesDF$sent, xlab="Year", ylab="ave sentence length")
